...
- **YouForge Flow popup rejects the Character lock value?** Confirm it's a UUID in 8-4-4-4-12 lowercase hex format. Copy the value from your saved Character's `<img>` element in Flow (the part after `?name=` in the `src` URL). See `docs/setup-guides/setup-google-flow.md` → "Step: Lock a character".
- **Image tasks fail with `BadCharacterLockError`?** The stored lock value is corrupt (popup validation should have prevented this; the most likely cause is a direct `chrome.storage.local` write). Re-copy the media ID from Flow and save it through the popup.
- **Is the character lock active?** Open the YouForge Flow service-worker console. Every image task logs `[api] Task <id> ingredient=<UUID>`. `ingredient=none` means the lock is unset for that profile.
