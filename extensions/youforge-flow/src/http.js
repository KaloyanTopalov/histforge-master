// YouForge Flow - HTTP primitives
// fetchWithTimeout: wraps fetch with an AbortController + setTimeout so a
// hung upstream request can't park a concurrency slot indefinitely.
// On timeout the AbortController.abort fires; the fetch rejects with an
// AbortError, which we rewrap as `TIMEOUT: <url>` so call sites and the
// retry helper can detect it from the error message alone.
//
// computeBackoff / retryWithBackoff: exponential + jittered retry loop.
// Default shouldRetry reads err.retryable (set by parseFlowApiError); call
// sites whose errors don't go through the parser (HistForge webhook,
// media-fetch) pass an explicit shouldRetry. STOP_REQUESTED and
// err.isSessionExpired are never retried.
//
...
// Exponential backoff: min(capMs, baseMs * 2^attempt), optionally
// scaled by a multiplicative jitter factor in [0.5, 1.5).
...
async function retryWithBackoff(fn, opts) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (e && e.isSessionExpired) throw e;
