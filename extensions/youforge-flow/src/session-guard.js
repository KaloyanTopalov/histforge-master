...
async function executeTaskWithSessionGuard(task, tabId, correlationId) {
  assertNotStopped();
  try {
    return await executeTaskViaAPI(task, tabId, correlationId);
  } catch (error) {
    if (error.message === 'STOP_REQUESTED') throw error;
    if (error.isSessionExpired) {
      await notifySessionExpired(error.message);
    }
    // Stash the correlation id on the Error so handlers/webhook can
    // forward it to HistForge — without this, runner.js's catch can't
    // tell which task lifetime the error came from. Don't overwrite if
    // a deeper layer already attached one.
