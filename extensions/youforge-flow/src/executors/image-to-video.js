...
async function runImageToVideo(task, ctx) {
  ...
  if (imageUrl && imageUrl.trim()) {
    ...
    log.safeLog('Uploading', imageUrls.length, 'reference image(s) for i2v...');
    for (let i = 0; i < imageUrls.length; i++) {
      try {
        const mediaId = await uploadImage(imageUrls[i], `ref_image_${i + 1}.png`);
        if (mediaId) refImageIds.push(mediaId);
      } catch (e) {
        log.safeLog(`[api] Ref image ${i + 1} upload failed:`, e.message);
      }
    }
  }

  if (settings.videoModelQuality === 'quality' && !modelKeys.isLite) {
    log.safeLog('Note: Ingredients mode does not support Veo 3.1 Quality, using Fast');
  }
  ...
  if (modelKeys.isLite) {
    ...
    log.safeLog('VEO Lite image-to-video, image:', refImageIds[0], 'model:', videoModelKey);
    ...
    log.safeLog('Image-to-video, refs:', refImageIds.length, 'model:', videoModelKey);
