...
const SETTINGS_SCHEMA = [
  ...
  // Character lock — a Google Flow saved-Character entity ID (UUID) the
  // operator captures from Flow's network calls and pastes into the
  // popup. When set, every image task adds
  // `referenceEntities: [{ entityId: <UUID> }]` to the
  // flowMedia:batchGenerateImages request (matching Flow's own UI wire
  // shape); no upload step. Empty means no lock. Coercion path rejects
  // '' (kind:'string' contract), so clearing the lock goes through the
  // dedicated setCharacterLockReference action in messages.js — same
  // escape hatch updateWebhooks uses for ''.
