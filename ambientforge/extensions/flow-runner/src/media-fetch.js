      ...
      for (let fetchAttempt = 1; fetchAttempt <= 3 && !fetched; fetchAttempt++) {
        try {
          ...
          if (fetchAttempt > 1) {
            ...
            safeLog(`[media] Media fetch retry ${fetchAttempt}/3, waiting 3s...`);
            await new Promise((r) => setTimeout(r, 3000));
            if (getStopFlag()) break;
          }
          const mf = await fetchOneMediaViaPage(freshTabs[0].id, url);
          mediaFiles.push(mf);
          safeLog(`[media] ✓ Fetched media: ${(mf.size / 1024).toFixed(0)}KB (${mf.mimeType})`);
          fetched = true;
        } catch (e) {
          if (e.mainWorldError) {
            safeLog(`[media] ⚠ Media fetch attempt ${fetchAttempt}/3 failed:`, e.message);
          } else {
            safeLog(`[media] ⚠ Media fetch attempt ${fetchAttempt}/3 error: ${e.message}`);
          }
        }
      }
    }
