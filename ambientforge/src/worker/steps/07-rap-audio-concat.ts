...
/**
 * Rap-compilation variant of step 07. Same concat-of-30-(or-N)-tracks logic
 * as the ambient step, but does NOT loop to a target duration — the natural
 * sum of song durations is what drives the final video length.
 *
 * Emits build/concat.wav (no loop.wav). Step 08 is skipped for rap
 * workflows; step 09-rap reads concat.wav directly.
 */
export type Step07RapOpts = {
  projectsDir?: string;
};

export const step07RapAudioConcat: PipelineStep = async (album, log) =>
  step07RapInternal(album, log);

export async function step07RapInternal(
  album: Album,
  log: LogFn,
  opts: Step07RapOpts = {},
): Promise<void> {
  log('step 07-rap', 'start');
  ...
  // Mark Branch B as in-flight at the very first opportunity.
  albumsRepo.patch(album.id, { videoStatus: 'rendering', videoProgressPct: 0 });

  // Test-only synthetic failure for runtime checks (matches ambient step 07).
  const forceFailFor = getRawSetting('force_branch_b_failure_for_album');
  if (forceFailFor && forceFailFor === album.id) {
    log(
      'step 07-rap',
      `force-fail flag matches album ${album.id} — throwing FORCE_BRANCH_B_FAILURE`,
    );
  ...
  if (tracks.length === 0) {
    throw new FfmpegError('STEP_07_RAP_NO_TRACKS', `album ${album.id} has zero tracks`);
  }
  const wavPaths: string[] = [];
  for (const t of tracks) {
    if (!t.audioPath) {
      throw new FfmpegError(
        'STEP_07_RAP_TRACK_AUDIO_MISSING',
